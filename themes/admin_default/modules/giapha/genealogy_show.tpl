...
<div id="module_show_list">
	...
	<div class="contextMenu" id="menu_genealogy_show">
		<ul>
			<li id="news1">
				<img src="{NV_BASE_SITEURL}js/contextmenu/icons/copy.png" /> Thêm Con
			</li>
			<li id="news2">
				<img src="{NV_BASE_SITEURL}js/contextmenu/icons/copy.png" /> Thêm Vợ
			</li>
			<li id="news3">
				<img src="{NV_BASE_SITEURL}js/contextmenu/icons/copy.png" /> Thêm chồng
			</li>
			<li id="edit">
				<img src="{NV_BASE_SITEURL}js/contextmenu/icons/rename.png" /> Sửa
			</li>
			<li id="delete">
				<img src="{NV_BASE_SITEURL}js/contextmenu/icons/delete.png" /> Xóa
			</li>
		</ul>
	</div>
</div>
		...
		$('#foldertree span').contextMenu('menu_genealogy_show', {
			...
			bindings : {
				'news1' : function(t) {
					...
					$("#modalIFrame").attr('src', script_name + '?' + nv_name_variable + '=' + nv_module_name + '&' + nv_fc_variable + '=users&gid={GID}&relationships=1&parentid=' + r_split[1]);
				},
				'news2' : function(t) {
					var r_split = t.id.split("_");
					$("div#create_genealogy_users").dialog({
					...
					$("#modalIFrame").attr('src', script_name + '?' + nv_name_variable + '=' + nv_module_name + '&' + nv_fc_variable + '=users&gid={GID}&relationships=2&parentid=' + r_split[1]);
				},
				'news3' : function(t) {
					var r_split = t.id.split("_");
					$("div#create_genealogy_users").dialog({
					...
					$("#modalIFrame").attr('src', script_name + '?' + nv_name_variable + '=' + nv_module_name + '&' + nv_fc_variable + '=users&gid={GID}&relationships=3&parentid=' + r_split[1]);
				},
				'edit' : function(t) {
					var r_split = t.id.split("_");
					$("div#create_genealogy_users").dialog({
					...
					$("#modalIFrame").attr('src', script_name + '?' + nv_name_variable + '=' + nv_module_name + '&' + nv_fc_variable + '=users&id=' + r_split[1]);
				},
				'delete' : function(t) {
					if(confirm('Bạn có chắc chắn xóa thành viên, xóa thành viên này hệ thống xẽ xóa tất cả các thành viên là vợ, con, cháu ..')) {
						var r_split = t.id.split("_");
						window.location = script_name + '?' + nv_name_variable + '=' + nv_module_name + '&' + nv_fc_variable + '=genealogy_show&&gid={GID}&deleteid=' + r_split[1];
					}
				}
			}
		});
...
<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		$("#modalIFrame").attr('src', script_name + '?' + nv_name_variable + '=' + nv_module_name + '&' + nv_fc_variable + '=users&gid={GID}&parentid=0');
	});
	//]]>
</script>
<!-- END: create_users -->
